...
这个设计的核心动机，是把“正在使用的本机”与“高负载转写任务”拆开。

如果把转写完全放在本机运行，`base`、`small` 这类模型在很多电脑上还能接受；但模型继续变大、质量继续提高后，就会更明显地占用 CPU、内存、磁盘 I/O 甚至电池，让正在使用电脑的人难以同时做别的事。把转写放到远端服务器后，本地客户端就能把资源留给浏览器、编辑器、会议软件、代码环境和 GUI 审校工作区。

CS 架构还有一个现实好处：可以按机器能力分层选模型。

- 2 核 / 2 GB 这种小服务器，先用 `tiny`
- 稍强一点的 CPU 服务器，可以尝试 `base` 或 `small`
- 更强的高内存或 GPU 机器，可以专门承担更大的模型
- cookies、URL 下载和长时间任务也可以集中到服务器端统一维护，而不是每台客户端都单独配置

当前服务端策略之所以保守，是因为小机器更怕“多个重任务一起进来”。所以现在的设计是：

- HTTP 控制面可以并发响应轻量请求
- 重型转写任务默认仍然只允许 1 个活跃任务

这样比起同时接受多个任务、最后一起被内存压垮，更适合低配服务器。

从更长远的规划看，这条边界也为“边缘节点”留了空间。如果未来运行时条件允许，手机、平板、迷你主机、闲置笔记本都有可能成为 FlowScribe 的服务端节点。不过这属于后续规划，不代表现在已经提供了现成的移动端服务端打包方案。
